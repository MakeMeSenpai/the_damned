...
class PlayPauseButton(Button):
    def __init__(self, pause_img, x, y):
        self.img = pause_img
        self.x = x
        self.y = y
        self.width = self.img.get_width()
...
class Menu:
    ...
    def draw(self, win):
        ...
        for item in self.buttons:
            item.draw(win)
            # win.blit([some-asset], (item.x + item.width + 5, item.y-9))
            text = self.font.render(str(self.item_cost[self.tower.level - 1]), 1, (255,255,255))
            win.blit(text, (item.x + item.width + 30 - text.get_width()/2, item.y + 80 -8)) #[some-asset].get_height() -8))
...
class VerticalMenu(Menu):
    ...
    def draw(self, win):
        ...
        for item in self.buttons:
            item.draw(win)
            # win.blit([some-asset], (item.x+2, item.y + item.height))
